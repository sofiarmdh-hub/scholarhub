...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "UPDATE pendaftaran_lomba SET status = ?, catatan_admin = ? WHERE id = ?";
    $stmt = mysqli_prepare($conn, $sql);
    
    if ($stmt) {
        mysqli_stmt_bind_param($stmt, "ssi", $status, $catatan, $id_pendaftaran);
        if (mysqli_stmt_execute($stmt)) {
            echo "<script>alert('Status berkas pendaftaran berhasil diperbarui!'); window.location='../pages/data-pendaftaran.php';</script>";
        } else {
